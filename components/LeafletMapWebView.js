...
<style>
  ...
  .legend {
    ...
    box-shadow: 0 2px 6px ${legendShadow};
    z-index: 9999;
    color: ${legendText};
  }
...
<script>
  ...
  L.tileLayer('${tileUrl}', { maxZoom: 19, attribution: '${tileAttribution}' }).addTo(window.map);
  ...
  function colorForRain(mm){ if (mm==null) return '#9CA3AF'; if (mm===0) return '#A3A3A3'; if (mm<=1) return '#93C5FD'; if (mm<=5) return '#60A5FA'; if (mm<=10) return '#3B82F6'; if (mm<=20) return '#2563EB'; return '#1D4ED8'; }
  function rForRain(mm){ return mm==null?5:Math.min(12,5+Math.sqrt(mm)*2); }
  window.rainLayer=L.layerGroup();
  function renderRainfall(points){ if(!Array.isArray(points))return; window.rainLayer.clearLayers(); points.forEach(p=>{ if(!Number.isFinite(p.lat)||!Number.isFinite(p.lon))return; const mm=(p.value==null||isNaN(p.value))?null:Number(p.value); const c=colorForRain(mm); const r=rForRain(mm); L.circleMarker([p.lat,p.lon],{radius:r,color:c,fillColor:c,fillOpacity:0.85,weight:1}).addTo(window.rainLayer).bindPopup('<b>'+(p.name||p.id||'Station')+'</b><br/>Rainfall: '+(mm==null?'—':(mm+' mm'))); }); }
  ...
  function renderWind(points){ if(!Array.isArray(points))return; window.windLayer.clearLayers(); points.forEach(p=>{ if(!Number.isFinite(p.lat)||!Number.isFinite(p.lon))return; const kt=(p.value==null||isNaN(p.value))?null:Number(p.value); const c=colorForWind(kt); const r=rForWind(kt); L.circleMarker([p.lat,p.lon],{radius:r,color:c,fillColor:c,fillOpacity:0.9,weight:1}).addTo(window.windLayer).bindPopup('<b>'+(p.name||p.id||'Station')+'</b><br/>Wind: '+(kt==null?'—':(kt.toFixed(1)+' kt'))); }); }
  ...
  function renderTemp(points){ if(!Array.isArray(points))return; window.tempLayer.clearLayers(); points.forEach(p=>{ if(!Number.isFinite(p.lat)||!Number.isFinite(p.lon))return; const v=(p.value==null||isNaN(p.value))?null:Number(p.value); const c=colorForTemp(v); const r=rForTemp(v); L.circleMarker([p.lat,p.lon],{radius:r,color:c,fillColor:c,fillOpacity:0.9,weight:1}).addTo(window.tempLayer).bindPopup('<b>'+(p.name||p.id||'Station')+'</b><br/>Temp: '+(v==null?'—':(v.toFixed(1)+' °C'))); }); }
  ...
  function renderHum(points){ if(!Array.isArray(points))return; window.humLayer.clearLayers(); points.forEach(p=>{ if(!Number.isFinite(p.lat)||!Number.isFinite(p.lon))return; const v=(p.value==null||isNaN(p.value))?null:Number(p.value); const c=colorForHum(v); const r=rForHum(v); L.circleMarker([p.lat,p.lon],{radius:r,color:c,fillColor:c,fillOpacity:0.9,weight:1}).addTo(window.humLayer).bindPopup('<b>'+(p.name||p.id||'Station')+'</b><br/>RH: '+(v==null?'—':(v.toFixed(0)+' %'))); }); }
  ...
  function renderPM(points){ if(!Array.isArray(points))return; window.pmLayer.clearLayers(); points.forEach(p=>{ if(!Number.isFinite(p.lat)||!Number.isFinite(p.lon))return; const v=(p.value==null||isNaN(p.value))?null:Number(p.value); const c=colorForPM(v); const r=rForPM(v); L.circleMarker([p.lat,p.lon],{radius:r,color:c,fillColor:c,fillOpacity:0.9,weight:1}).addTo(window.pmLayer).bindPopup('<b>'+(p.name||p.id||'Region')+'</b><br/>PM2.5: '+(v==null?'—':(v.toFixed(0)+' µg/m³'))); }); }
  ...
  window.dengueLayer=L.geoJSON(null,{style:function(f){ var s=dengueCaseSize(f.properties||{}); var c=colorForDengue(s); return {color:c,weight:2,fillColor:c,fillOpacity:0.25}; }, onEachFeature:function(feature,layer){ try{ var props=feature.properties||{}; var size=dengueCaseSize(props); var loc=dengueLocality(props); var link=dengueLink(props); var html='<b>'+loc+'</b><br/>Cases: '+(size==null?'—':size); if(link) html+='<br/><a href=\"'+link+'\" target=\"_blank\" rel=\"noopener noreferrer\">More</a>'; layer.bindPopup(html);}catch(e){} }});
  function renderDengue(geojson){ try{ window.dengueLayer.clearLayers(); if(geojson&&(geojson.type==='FeatureCollection'||geojson.type==='Feature')){ window.dengueLayer.addData(geojson);} }catch(e){} }
  ...
  try { window.showOverlay(${overlayJSON}); } catch(e){}
    ...
    lat, lon, zoom, showMarker, pins, interactive,
    rainfallPoints, windPoints, tempPoints, humPoints, pmPoints,
    dengueGeoJSON, overlay, legendBottom, showLegend, dark,
  ]);
  ...
  useImperativeHandle(ref, () => ({
    ...
    setRainfall: (pointsArray) => {
      ...
      webRef.current.injectJavaScript(`
        try { if (window.updateRainfall) { window.updateRainfall(${JSON.stringify(payload)}); } } catch (e) {}
        true;
      `);
    },
    setWind: (pointsArray) => {
      ...
      const payload = JSON.stringify(pointsArray || []);
      webRef.current.injectJavaScript(`
        try { if (window.updateWind) { window.updateWind(${JSON.stringify(payload)}); } } catch (e) {}
        true;
      `);
    },
    setTemp: (pointsArray) => {
      ...
      const payload = JSON.stringify(pointsArray || []);
      webRef.current.injectJavaScript(`
        try { if (window.updateTemp) { window.updateTemp(${JSON.stringify(payload)}); } } catch (e) {}
        true;
      `);
    },
    setHum: (pointsArray) => {
      ...
      const payload = JSON.stringify(pointsArray || []);
      webRef.current.injectJavaScript(`
        try { if (window.updateHum) { window.updateHum(${JSON.stringify(payload)}); } } catch (e) {}
        true;
      `);
    },
    setPM: (pointsArray) => {
      ...
      const payload = JSON.stringify(pointsArray || []);
      webRef.current.injectJavaScript(`
        try { if (window.updatePM) { window.updatePM(${JSON.stringify(payload)}); } } catch (e) {}
        true;
      `);
    },
    setDengue: (geojson) => {
      ...
      webRef.current.injectJavaScript(`
        try { if (window.updateDengue) { window.updateDengue(${JSON.stringify(payload)}); } } catch (e) {}
        true;
      `);
    },
    setOverlay: (kind) => {
      if (!webRef.current) return;
      webRef.current.injectJavaScript(`
        try { if (window.showOverlay) { window.showOverlay(${JSON.stringify(kind || "rain")}); } } catch (e) {}
        true;
      `);
    },
  }), [zoom]);
  ...
  useEffect(() => {
    if (!webRef.current || !webReadyRef.current) return;
    webRef.current.injectJavaScript(`
      try { if (window.showOverlay) { window.showOverlay(${JSON.stringify(overlay || "rain")}); } } catch (e) {}
      true;
    `);
  }, [overlay]);
  ...
  return (
    <View style={{ height, overflow: "hidden", borderRadius: 16 }}>
      <WebView
        ...
        style={{
          flex: 1,
          backgroundColor: Platform.select({ ios: "transparent", android: "transparent" }),
        }}
