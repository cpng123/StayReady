...
// Keep timers predictable for setTimeout used
...
jest
  ...
  .mockImplementation((value, cfg = {}) => immediate(value, cfg.toValue));
jest
...
// expo-av mock (functions defined inside the factory)
jest.mock("expo-av", () => {
...
describe("SplashScreen (integration)", () => {
  ...
  test('plays intro SFX when pref enabled ("1") and navigates to Main', async () => {
    ...
    await act(async () => {
      await Promise.resolve();
    }); // sometimes one more tick helps
    // Now fast-forward past the kickoff delay
    await act(async () => {
      jest.advanceTimersByTime(801);
    });
    expect(expoAv.setPositionAsync).toHaveBeenCalledTimes(1);
